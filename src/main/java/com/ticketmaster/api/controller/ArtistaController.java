...
public class ArtistaController {
    ...
    private record IdempotentArtistaResponse(ArtistaFingerprint fingerprint, ArtistaResponse body, URI location) {}
    ...
    public ResponseEntity<EntityModel<ArtistaResponse>> criar(
        ...
        ArtistaFingerprint fingerprint = new ArtistaFingerprint(req.getNome(),
        ...
        synchronized (idempotencyLock) {
            ...
            if (artistaRepository.findByNomeContainingIgnoreCase(req.getNome(), Pageable.unpaged())
            ...
            EntityModel<ArtistaResponse> body = assembler.toModel(toResponse(salvo));
            URI location = URI.create("/artistas/" + salvo.getId());
            idempotencyCache.put(idempotencyKey, new IdempotentArtistaResponse(fingerprint, toResponse(salvo), location));
    ...
    public ResponseEntity<Void> deletar(@PathVariable Long id) {
        if (!artistaRepository.existsById(id)) throw new ResourceNotFoundException("Artista", id);
    ...
    private Artista toEntity(ArtistaRequest req) {
        ...
        a.setNome(req.getNome()); a.setGenero(req.getGenero());
        a.setBio(req.getBio()); a.setUrlImagem(req.getUrlImagem());
        return a;
    }

    private ArtistaResponse toResponse(Artista a) {
        ArtistaResponse r = new ArtistaResponse();
        r.setId(a.getId()); r.setNome(a.getNome()); r.setGenero(a.getGenero());
        r.setBio(a.getBio()); r.setUrlImagem(a.getUrlImagem());
